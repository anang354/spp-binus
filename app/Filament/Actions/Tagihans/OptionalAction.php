...
class OptionalAction 
{
    ...
    {
        return Action::make('optional')
            ->label('Buat Tagihan Optional')
            ->color('success')
            ...
            ->form([
                Grid::make()
                    ...
                    ->schema([
                        Placeholder::make('info')->color('danger')->content('Tagihan akan dibuat untuk siswa yang dipilih saja berdasarkan biaya dan diskon yang telah ditentukan')->columnSpanFull(),
                        ...
                        Select::make('biaya')
                            ...
                            ->columnSpan(4)
                            ->required(),
                        Select::make('siswa')->options(function (Get $get): array {
                            $jenjang = $get('jenjang');
                            return \App\Models\Siswa::whereHas('kelas', function ($query) use ($jenjang) {
            ...
            ->action(function (array $data){
                ...
                $getSiswa = \App\Models\Siswa::query()
                            ...
                            ->with(['diskon.biaya', 'kelas']) // Eager loading untuk efisiensi database
                            ->get();
                DB::beginTransaction();
                try {
                    $hitung = 0;
